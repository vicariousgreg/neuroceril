...
class State:
    ...
    @staticmethod
    def from_tuple(tup):
        return State(tup[0], sym=tup[1],
            ctx_state=(None if tup[2] is None else
                State(tup[2], sym=tup[3])))
...
class Connection:
    ...
    def activate(self):
        ...
        if self.emulate:
            ...
            sym = em_out.get_symbol()

            self.to_layer.activ_history[sym] = self.to_layer.activ_history.get(sym, 0) + 1

            #self.activ_history[sym] = self.activ_history.get(sym, 0) + 1
            pair = (self.from_layer.outputs.get_symbol(), sym)
            self.activ_history[pair] = self.activ_history.get(pair, 0) + 1
    ...
    def emulate_flash(self,
        ...
        if to_ctx_layer:
            ...
            to_ctx_syms = [None for _ in to_syms]

        if from_ctx_layer:
            for sym in from_ctx_syms:
                from_ctx_layer.encode(sym)
        else:
            from_ctx_syms = [None for _ in to_syms]
...
class Network:
    ...
    def run_auto(self, t=100, verbose=False, debug=False, inputs=(), callback=None):
